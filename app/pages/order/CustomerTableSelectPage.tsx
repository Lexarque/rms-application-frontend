...
export default function CustomerTableSelectPage() {
  const navigate = useNavigate();
  const { session, setSession } = useOrderSession();
  const { data: tables = [], isLoading, isError } = useRestaurantTables();
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "40px 24px" }}>
      <SectionHeader
        ...
        subtitle="Pick an available table for your order"
      />
      ...
      {isError ? (
        <p style={{ fontFamily: font.body, color: C.danger }}>Error loading tables.</p>
      ) : isLoading ? (
